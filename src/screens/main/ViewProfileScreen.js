              ...
              Navigation.showModal({
                component: {
                  ...
                  passProps: {
                    ...
                    runAction: () =>
                      profileActions.fetchProfileFollowers(profile.profile_id),
                    loadMore: () =>
                      profileActions.fetchMoreProfileFollowers(
                        profile.profile_id,
                      ),
                    screentype: 'modal',
                  },
              ...
              Navigation.showModal({
                component: {
                  ...
                  passProps: {
                    ...
                    runAction: () =>
                      profileActions.fetchProfilesFollowing(profile.profile_id),
                    loadMore: () =>
                      profileActions.fetchMoreProfilesFollowing(
                        profile.profile_id,
                      ),
                    screentype: 'modal',
                  },
              ...
              Navigation.showModal({
                component: {
                  ...
                  passProps: {
                    ...
                    runAction: () =>
                      profileActions.fetchKnownProfileFollowers(
                        profile.profile_id,
                      ),
                    loadMore: () =>
                      profileActions.fetchMoreKnownProfileFollowers(
                        profile.profile_id,
                      ),
                    screentype: 'modal',
                  },
...
const ViewProfileScreen = ({
  ...
  let righticon =
    hideparallax == true ? (
      <Text style={{color: colors.blue}}>Restore</Text>
    ) : (
      <Icon
        type="entypo"
  ...
  function addProfilePost(data) {
    ...
    useowner
      ? addUserViewProfileFormPost(data)
      : addOthersViewProfileFormPost(data);
  }
  ...
  function setProfilePost(data) {
    ...
    useowner
      ? setUserViewProfileFormPost(data)
      : setOthersViewProfileFormPost(data);
  }
  ...
  function setRefresh(data) {
    ...
    useowner
      ? setProcessing(data, 'userviewprofileformrefreshing')
  ...
  function followProfile() {
    ...
    else num_followers = num_followers - 1 < 0 ? 0 : num_followers - 1;
    followProfileAction(
      toshowprofile.profile_id,
      () => {
  ...
  function handleProfilePostsFetch(start, end) {
    if (checkData(toshowprofile) && toshowprofile.profileblockedu == false) {
      ...
      fetchProfilePosts(
        ...
        () => {
          useowner
            ? setProcessing(true, 'userviewprofileformpostloading')
        ...
        (posts, profile, nexturl) => {
          ...
          setViewProfile(profile);
          useowner
            ? setUserViewProfileFormLink(nexturl)
            : setOthersViewProfileFormLink(nexturl);
          useowner
            ? setProcessing(false, 'userviewprofileformpostloading')
        ...
        action => {
          if (action == 'cancel') {
            ...
            useowner
              ? setProcessing('retry', 'userviewprofileformpostloading')
  ...
  function handleMoreProfilePostsFetch() {
    if (!checkData(viewprofileform.viewprofilepostsnexturl)) {
      useowner
        ? setProcessing('done', 'userviewprofileformpostloadingmore')
        : setProcessing('done', 'othersviewprofileformpostloadingmore');
      return;
    }
    fetchMoreProfilePosts(
      viewprofileform.viewprofilepostsnexturl,
      toshowprofile.profile_id,
      () => {
        useowner
          ? setProcessing(true, 'userviewprofileformpostloadingmore')
      ...
      action => {
        if (action == 'cancel') {
          ...
          useowner
            ? setProcessing('retry', 'userviewprofileformpostloadingmore')
  ...
  const renderView = () => {
    ...
    if (loaded == true) {
      ...
      torenderview = (
        <View style={{flex: 1, alignItems: 'center'}}>
          <PanelMsg message="Profile not found or cannot be shown" />
        </View>
      );
    }
