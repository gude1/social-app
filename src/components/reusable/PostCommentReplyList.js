...
export default class PostCommentReplyList extends Component {
  constructor(props) {
    ...
    if (
      this.props.userprofile.profile_id == this.props.origin.profile.profile_id
    ) {
      this.bottomodallistothersoptions = [
        ...this.bottomodallistothersoptions,
        {
  ...
  _muteProfile = () => {
    let initaction = () =>
      this.props.setProcessing(true, 'postcommentreplyformmuting');
    let failedaction = () =>
      ...
      return (
        <View
          ...
          <Icon
            onPress={() =>
              this.props.onFetch(
                this.props.origin.commentid || this.props.origin.replyid,
              )
            }
            color={colors.text}
      ...
      <View style={styles.postTextContainer}>
        {
          <ListItem
                ...
                component: {
                  ...
                  passProps: {
                    ...
                    reqdata: checkData(this.props.origin.replyid)
                      ? {replyid: this.props.origin.replyid}
                      : {commentid: this.props.origin.commentid},
                  },
                },
              })
            }
            ...
            subtitle={
              this.props.origin.reply_text || this.props.origin.comment_text
            }
            ...
            BottomContainerItem={
              <View
                ...
                <Text
                  ...
                  {this.props.origin.num_replies} replies
                </Text>
              </View>
            }
          />
  ...
  _onPress = item => {
    if (
      !isEmpty(item) &&
      item.retry == true &&
      checkData(this.props.makeReply)
    ) {
      this.props.makeReply([
        this.props.origin.replyid || this.props.origin.commentid,
        item.reply_text,
        item.replyid,
      ]);
    } else {
  ...
  _renderItem = ({item}) => {
    if (
      ...
      return <PanelMsg message={'Reply is unavailable'} />;
    ...
    return (
      <ListItem
        ...
        onLongPress={() =>
          this._setSelected(item.replyid, item.profile.profile_id, item)
        }
        //replyPress={}
        ...
        likebtn
        likePress={() =>
          this._onItemLiked(item.replyid, item.replyliked, item.num_likes)
        }
    ...
    return (
      <>
        <FlatList
          ...
          ListFooterComponent={
            this.props.data.length > 0 && this._setFooterComponent()
          }
        ...
        <ConfirmModal
          ...
          confirmMsg={
            this.state.replyhidden == true ? 'Unhide reply?' : 'Hide reply?'
          }
          acceptText="Yeah"
          acceptAction={() => {
