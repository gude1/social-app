...
function GalleryImage({
  ...
  return (
    <TouchableOpacity onPress={() => onSelect(d.caption)}>
      <Image
        source={{uri: d.photo}}
        containerStyle={{
...
const GalleryList = ({
  ...
  const showContent = () => {
    ...
    return photoList.length == 0 ? (
      ...
      <ScrollView refreshing={true} showsVerticalScrollIndicator={false}>
        <View
          style={[
            {
              flex: 1,
  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      ...
      <View style={styles.otherSectionStyle}>{showContent()}</View>
    </SafeAreaView>
  );
};
