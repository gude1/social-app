...
export class Header extends React.PureComponent {
  render() {
    ...
    lefticon =
      lefticon == null || lefticon == undefined || lefticon == '' ? null : (
        <TouchableScale
          style={{
            marginHorizontal: 5,
    ...
    righticon =
      righticon == null || righticon == undefined || righticon == '' ? null : (
        <TouchableScale
          style={{
            marginHorizontal: 5,
            //borderWidth: 2,
...
export class HeaderWithImage extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState, nextContext) {
    if (
      this.props?.avatarUri?.uri != nextProps?.avatarUri?.uri ||
      ...
      <View
        ...
        {this.props.buttonTitle && (
          <TouchableScale onPress={this.props.buttonPress}>
            <Text style={{color: colors.blue}}>{this.props.buttonTitle}</Text>
          </TouchableScale>
        )}
      </View>
    );
...
export class ImageViewPager extends React.Component {
  constructor(props) {
    super(props);
    this.pageritems = this._createItems(
      this.props.images,
...
export class BottomListModal extends Component {
  ...
  _renderList = () => {
    ...
    let list = listData.map((listitem, index) => {
      return (
        <TouchableOpacity onPress={listitem.onPress} key={index}>
    ...
    <Overlay
      ...
      <View style={styles.confirmModalCard}>
        ...
        <View style={styles.confirmModalBtnCtn}>
          <TouchableOpacity onPress={acceptAction}>
            <Text style={styles.confirmModalBtn}>
              {!!acceptText ? acceptText : 'Ok'}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={rejectAction}>
            <Text style={styles.confirmModalBtn}>
    ...
    <Overlay
      ...
      <Animatable.View animation={'bounceInUp'} useNativeDriver={true}>
        <View style={styles.avatarNavModal}>
          ...
          <View style={styles.avatarNavModalNavCtn}>{renderNavItems()}</View>
        </View>
      </Animatable.View>
    </Overlay>
  );
...
export class ListItem extends Component {
  ...
  _setBtnComponent = () => {
    if (this.props.likebtn == true && this.props.hide != true) {
      return (
        this.props.likeButtonComponent || (
          <Icon
        ...
        <View style={styles.listItemStyle}>
          ...
          <View style={styles.listItemTextCtn}>
            ...
            <View style={styles.listItemBottomCtnStyle}>
              {checkData(time) ? (
                <Text style={[styles.bottomCtnLeftText, timeTextStyle]}>
                  {time}
                </Text>
              ) : null}
              {checkData(likes) ? (
                <Text onPress={numLikesPress} style={styles.bottomCtnRightText}>
                  {likes} likes
                </Text>
              ) : null}
              {replyPress && this.props.hide == false ? (
                <Text onPress={replyPress} style={styles.bottomCtnRightText}>
                  Reply{' '}
                </Text>
              ) : null}
              {checkData(replies) ? (
...
export class ImageGalleryItem extends Component {
  ...
  render() {
    ...
    let smallavataruri =
      typeof smallavatar == 'string' ? {uri: smallavatar} : smallavatar;
      ...
      return (
        <View
          ...
          <Icon
            ...
            size={responsiveFontSize(5)}
          />
          <Text style={{margin: 5, color: colors.iconcolor}}>
            Nothing to show
          </Text>
        </View>
      );
...
export function optimizeComponent(WrappedComponent, update) {
  return class Optimize extends Component {
    shouldComponentUpdate(nextProps, nextState, nextContext) {
      return checkData(update) ? update(nextProps, this.props) : false;
    }
    render() {
