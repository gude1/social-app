...
class MyListHeader extends Component {
  shouldComponentUpdate(nextProps, nextState, nextContext) {
    if (
      this.props.isnumselect != nextProps.isnumselect ||
  ...
  _turnOnSelect = () => {
    return this.props.isnumselect
      ? this._passSelectedData(this.props.itemsselected)
      : this.props.turnOnSelect();
  };
  render() {
    const {
      colors,
...
export default class PhotoGallery extends React.PureComponent {
  ...
  _onPressItem = (id) => {
    ...
    this.setState((state) => {
      ...
      if (selected.has(id)) {
        ...
        selected.has(id) == false &&
        !(state.selected.size >= this.props.maxSelect)
      ) {
        selected.set(id);
        isnumselect = true;
      }
  ...
  _getItemLayout = (data, index) => ({
    length: this.props.dimensions.reswidth,
    offset: this.props.dimensions.reswidth * index,
    index,
  });

  render() {
    const {colors, dimensions, photoList, onCancel, onDisplayImages} =
      this.props;
