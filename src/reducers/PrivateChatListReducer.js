...
const makeList = (data: Array) => {
  ...
  return data.sort(
    (item1, item2) => item1.chats[0].created_at - item2.chats[0].created_at,
  );
};
...
const updateChats = (state, payload) => {
  ...
  let chatlistnewstate = state.chatlist.map((item) => {
    if (
      item.create_chatid == payload.create_chatid ||
...
const PrivateChatListReducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case UPDATE_PRIVATECHATLIST_CHATS:
      ...
      let findchatobj = state.chatlist.find(
        (item) =>
          item.create_chatid == action.payload.create_chatid ||
          item.partnerprofile.profile_id ==
            action.payload.partnerprofile.profile_id,
      ...
      reducerdata = state.chatlist.map((item) => {
        ...
        if (
          ...
          newchats = item.chats.map((chatitem) => {
            return chatitem.id == action.payload.id
    ...
    case UPDATE_ARRAY_PRIVATECHATLIST:
      ...
      let updatestate = state.chatlist.map((item) => {
        let value = action.payload.find(
          (newitem) => newitem.create_chatid == item.create_chatid,
        );
    ...
    case DELETE_PRIVATECHATLIST:
      ...
      let newstate = state.chatlist.map((item) => {
        if (
          item.create_chatid == action.payload ||
    ...
    case UNPIN_PRIVATECHATLIST:
      ...
      reducerdata = state.pinnedchatarr.filter(
        (create_chatid) => create_chatid != action.payload,
      );
