...
const PrivateChatReducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case ADD_PRIVATECHAT:
      ...
      toaddpayloads = action.payload.filter(
        toadditem => !excluded_ids.includes(toadditem.id),
      );
    ...
    case ADD_PRIVATECHAT_LAST_FETCH_ARR:
      ...
      return {
        ...state,
        last_fetch_arr: [...state.last_fetch_arr, ...action.payload],
      };
      break;
    case REMOVE_PRIVATECHAT_LAST_FETCH_ARR:
      confirmId(state, action.create_chatid);
      reducerdata = state.last_fetch_arr.filter(
    ...
    case SET_PRIVATECHAT_INFO:
      ...
      return {
        ...state,
        privatechatinfo: {...state.privatechatinfo, ...action.payload},
      };
      break;
    case PROCESSING:
